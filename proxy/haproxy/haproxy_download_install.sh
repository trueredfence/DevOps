...
frontend pub1
        #bind :80 name clear
        bind :443 ssl crt /etc/ssl/changme.crt no-sslv3 alpn h2,http/1.1 # Change Certificate
        http-after-response set-header Strict-Transport-Security "max-age=31536000"
        http-request redirect scheme https unless { ssl_fc }
        mode http
        acl domain-1 hdr(host) -i changeme # Change domain
...
backend backend-1
        ...
        balance roundrobin
        mode http
        server webserver1 212.24.104.168:443 check weight 1 maxconn 50 ssl verify none option h1-case-adjust-bogus-server        
        cache cache
        total-max-size 200       
